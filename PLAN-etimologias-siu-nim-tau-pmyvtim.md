# Plano — Etimologias do Siu Nim Tau (PMYVTIM 2017)

Fonte: `E:/ving-tsun-system-guide/fontes/PMYVTIM2017-PIA-SNT-04.02.2017.txt`

Programação Inicial de Acesso do Domínio Siu Nim Tau 小念頭 — atualização 04.02.2017. Levantamento: 52 ideogramas únicos no texto-fonte, 34 já cobertos por notas existentes em `content/notes/etimologia-*.md`, 18 a pesquisar.

## Protocolo de execução

- **Uma etimologia por vez**. Sem paralelização de fetches no mesmo site.
- Skill: `research-chinese-etymology`. 7 fontes (MDBG, chardb/xiaoxue, hanziyuan, shuowen.org, zdic, CantoDict, e a 7ª conforme skill).
- Após criar a nota, marcar a checkbox aqui e commitar — **um commit por nota**, sem `Co-Authored-By`.
- Retomar do próximo caractere não marcado ao abrir nova sessão.

## Já existentes (não recriar)

Cobertura completa, inclusive em compostos:

一, 三, 中, 二, 伏, 內, 單, 圈, 字, 小, 念, 手, 打, 拳, 拿, 掌, 攤, 標, 橋, 法, 羊, 膀, 衝, 護, 走, 鉗, 長, 門, 開, 雙, 頭, 馬, 高, 黐

## Batch 1 — Técnicas de mão (6)

- [x] 掂 (dim) — vertical/firme; Dim Jeung 掂掌 (palma vertical)
- [x] 包 (baau) — embrulhar/envolver; Baau Sau 包手 (mão que envolve)
- [x] 窒 (jat) — obstruir/sufocar; Jat Sau 窒手
- [x] 收 (sau) — recolher/retrair; Sau Kuen 收拳 (recolher o punho)
- [x] 拍 (paak) — bater/tapear; Paak Sau 拍手, Paak Da 拍打
- [x] 耕 (gaang) — arar/lavrar; Gaang Sau 耕手

## Batch 2 — Direção e golpe (6)

- [x] 低 (dai) — baixo; Dai Waang Jeung 低橫掌 (palma horizontal baixa)
- [x] 橫 (waang) — horizontal; Go/Dai Waang Jeung 高/低橫掌
- [x] 直 (jik) — reto/direto; Jik Chung 直衝 (impulso reto)
- [x] 破 (poh) — quebrar/romper; Poh Jung Sau 破中手 (mão que rompe o centro)
- [x] 甩 (lat) — sacudir/desvencilhar; Lat Sau Jik Chung 甩手直衝
- [x] 日 (yat) — sol/dia; Yat Ji Jung Choei 日字中拳 (punho-caractere "sol" no centro)

## Batch 3 — Estrutura programática e troca (6)

- [x] 第 (dai) — ordinal; Dai Yat Jit 第一節 (Primeira Seção)
- [x] 節 (jit) — seção/nó; Dai Yat/Yi/Saam Jit 第一/二/三節
- [x] 接 (jip) — receber/conectar; Jip Sau 接手
- [x] 滾 (gwan) — rolar; Gwan Sau 滾手
- [ ] 轆 (luk) — rolar/roldana; Luk Sau 轆手
- [ ] 外 (ngoi) — fora/externo; Ngoi Mun 外門, Noi Ngoi Mun 內外門

Total: 18 etimologias a produzir.
