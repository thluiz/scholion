...
captured_at: "2025-08-04T17:06:45+01:00"
...
[![deep agent](https://github.com/hwchase17/deepagents/raw/master/deep_agents.png)](https://github.com/hwchase17/deepagents/blob/master/deep_agents.png)
...
See [examples/research/research\_agent.py](https://github.com/hwchase17/deepagents/blob/master/examples/research/research_agent.py) for a more complex example.
...
The second argument to `create_deep_agent` is `instructions`. This will serve as part of the prompt of the deep agent. Note that there is a [built in system prompt](https://github.com/hwchase17/deepagents?ref=blog.langchain.com&utm_source=tldrnewsletter#built-in-prompt) as well, so this is not the _entire_ prompt the agent will see.

### `subagents` (Optional)

A keyword-only argument to `create_deep_agent` is `subagents`. This can be used to specify any custom subagents this deep agent will have access to. You can read more about why you would want to use subagents [here](https://github.com/hwchase17/deepagents?ref=blog.langchain.com&utm_source=tldrnewsletter#sub-agents)
...
By default, `deepagents` will use `"claude-sonnet-4-20250514"`. If you want to use a different model, you can pass a [LangChain model object](https://python.langchain.com/docs/integrations/chat/).
...
`deepagents` comes with the built-in ability to call sub agents (based on Claude Code). It has access to a `general-purpose` subagent at all times - this is a subagent with the same instructions as the main agent and all the tools that is has access to. You can also specify [custom sub agents](https://github.com/hwchase17/deepagents?ref=blog.langchain.com&utm_source=tldrnewsletter#subagents--optional-) with their own instructions and tools.
...
-   Benchmark the example of [deep research agent](https://github.com/hwchase17/deepagents/blob/master/examples/research/research_agent.py)
