...
captured_at: "2026-08-12T16:17:02+01:00"
...
If I had to hand a leader one line from the paper, it's this: set the bar at the eval, not the demo. A demo shows an agent can work once. An eval suite with a real rubric shows it works reliably.
...
Behind that one instruction it scaffolds the project, writes the code, generates an eval set, runs it, deploys to a managed runtime, and reports back. Coordination between agents runs on open standards: MCP for tools, A2A for handing work to other agents.
