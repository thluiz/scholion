...
captured_at: "2026-09-24T23:13:41+01:00"
title: "Why I still self host my servers (and what I've recently learned)"
domain: "chollinger-com"
---

1.  [Introduction](#introduction)
2.  [My Services](#my-services)
3.  [Why I self host](#why-i-self-host)
4.  [Reasoning about complex systems](#reasoning-about-complex-systems)
5.  [Things that broke in the last 6 months](#things-that-broke-in-the-last-6-months)
6.  [Things I learned (or recalled) in the last 6 months](#things-i-learned-or-recalled-in-the-last-6-months)
    1.  [You can self host VS Code](#you-can-self-host-vs-code)
    2.  [UPS batteries die silently and quicker than you think](#ups-batteries-die-silently-and-quicker-than-you-think)
    3.  [Redundant DNS is good DNS](#redundant-dns-is-good-dns)
    4.  [Raspberry PIs run ARM, Proxmox does not](#raspberry-pis-run-arm-proxmox-does-not)
    5.  [zfs + Proxmox eat memmory and will OOM kill your VMS](#zfs--proxmox-eat-memmory-and-will-oom-kill-your-vms)
    6.  [The mystery of random crashes (Is it hardware? It’s always hardware.)](#the-mystery-of-random-crashes-is-it-hardware-its-always-hardware)
    7.  [SNMP(v3) is still cool](#snmpv3-is-still-cool)
    8.  [Don’t trust your VPS vendor](#dont-trust-your-vps-vendor)
    9.  [Gotta go fast](#gotta-go-fast)
    10.  [CIFS is still not fast](#cifs-is-still-not-fast)
    11.  [Blob storage, blob fish, and file systems: It’s all “meh”](#blob-storage-blob-fish-and-file-systems-its-all-meh)
    12.  [CrowdSec](#crowdsec)
7.  [Conclusion](#conclusion)

## Introduction[↑](#top "Back to top")

I self host everything but email. I wrote about this [here](https://chollinger.com/blog/2019/04/building-a-home-server/), [here](https://chollinger.com/blog/2023/04/migrating-a-home-server-to-proxmox-truenas-and-zfs-or-how-to-make-your-home-network-really-complicated-for-no-good-reason/), or [here](https://chollinger.com/blog/2023/10/moving-a-proxmox-host-with-a-sas-hba-as-pci-passthrough-for-zfs--truenas/).
...
![For those unfamiliar: Outdoors! (Sample image from our garden)](https://chollinger.com/_astro/image-20240825135445550.CWPtCcjU.png)
...
## Why I self host[↑](#top "Back to top")
...
## Reasoning about complex systems[↑](#top "Back to top")
...
![Here's a picture out our neighborhood barn cat, so you don't get too bored while we talk about protobuf compilers](https://chollinger.com/_astro/image-20240825131026074.Cz38XaLr.png)
...
## Things that broke in the last 6 months[↑](#top "Back to top")
...
## Things I learned (or recalled) in the last 6 months[↑](#top "Back to top")
...
### You can self host VS Code[↑](#top "Back to top")

This is a simple, but neat one: The OS part of VS Code is something that runs in a browser. Same concept that powers GitHub’s codespaces, but self hosted: [https://github.com/coder/code-server](https://github.com/coder/code-server)

Very useful to use VS Code on a device like an iPad or a Mac (or Windows) that wants a Linux box. Mine runs on `Ubuntu`.

![Useful on an iPhone? Eh...](https://chollinger.com/_astro/image-20240825131634407.DUL_7Gi7.png)
...
### UPS batteries die silently and quicker than you think[↑](#top "Back to top")
...
### Redundant DNS is good DNS[↑](#top "Back to top")
...
![DNS Mapping](https://chollinger.com/_astro/image-20240607214429699.krwTDjV3.png)
...
### Raspberry PIs run ARM, Proxmox does not[↑](#top "Back to top")
...
![Promox nodes](https://chollinger.com/_astro/image-20240607214732963.CYBte9VN.png)
...
### `zfs` + Proxmox eat memmory and will OOM kill your VMS[↑](#top "Back to top")

Did you know OOM kills were “a thing” in `$currentYear`? Sure are! I run `TrueNAS Scale` with a [SAS HBA in PCI passthru mode](https://chollinger.com/blog/2023/10/moving-a-proxmox-host-with-a-sas-hba-as-pci-passthrough-for-zfs--truenas/) and had my machines OOM killed occasionally when both Proxmox backups + device backups where running.
...
```
echo 'options zfs zfs_arc_max="8589934592"' >> /etc/modprobe.d/zfs.confupdate-initramfs -u
```

This limits `zfs` to 8 GiB.

### The mystery of random crashes (Is it hardware? It’s always hardware.)[↑](#top "Back to top")
...
```
Error 4 occurred at disk power-on lifetime: 27469 hours (1144 days + 13 hours)  When the command that caused the error occurred, the device was active or idle.  After command completion occurred, registers were:  ER ST SC SN CL CH DH  -- -- -- -- -- -- --  04 51 00 ff ff ff 0f  Commands leading to the command that caused the error were:  CR FR SC SN CL CH DH DC   Powered_Up_Time  Command/Feature_Name  -- -- -- -- -- -- -- --  ----------------  --------------------  ea 00 00 00 00 00 a0 00   6d+03:38:11.728  FLUSH CACHE EXT  61 00 08 ff ff ff 4f 00   6d+03:38:11.726  WRITE FPDMA QUEUED  61 00 08 ff ff ff 4f 00   6d+03:38:11.726  WRITE FPDMA QUEUED  61 00 08 ff ff ff 4f 00   6d+03:38:11.725  WRITE FPDMA QUEUED  61 00 b0 ff ff ff 4f 00   6d+03:38:11.725  WRITE FPDMA QUEUED
```

…which then caused weird deadlocks and such and the machine became unresponsive and got killed.

![LibreNMS monitoring](https://chollinger.com/_astro/image-20240607215539705.IJ83n4xj.png)
...
### SNMP(v3) is still cool[↑](#top "Back to top")
...
> 
> [https://www.cisa.gov/news-events/alerts/2017/06/05/reducing-risk-snmp-abuse](https://www.cisa.gov/news-events/alerts/2017/06/05/reducing-risk-snmp-abuse)
...
![LibreNMS](https://chollinger.com/_astro/image-20240824103028884.CBvr73Vp.png)
...
### Don’t trust your VPS vendor[↑](#top "Back to top")
...
![External network path says no](https://chollinger.com/_astro/image-20240607221106538.BIlrzRFN.png)
...
![Ping says no internally](https://chollinger.com/_astro/image-20240607221115352.CuAwnCfa.png)
...
![VPS Benchmark 1/2](https://chollinger.com/_astro/GPGtDkkXoAAW9XD.45K_Nm5C.png)
...
![VPS Benchmark 2/2](https://chollinger.com/_astro/GPGtEuXXYAAdzJ4.CXfsdD-K.png)
...
### CIFS is still not fast[↑](#top "Back to top")
...
![Storage Benchmark 1/2](https://chollinger.com/_astro/image-20240825111920987.VIta3UAo.png)
...
```
SIZE="256M"# Sequentialfio --name=job-r --rw=read --size=$SIZE --ioengine=libaio --iodepth=4 --bs=128K --direct=1fio --name=job-w --rw=write --size=$SIZE --ioengine=libaio --iodepth=4 --bs=128k --direct=1# Randomfio --name=job-randr --rw=randread --size=$SIZE --ioengine=libaio --iodepth=32 --bs=4K --direct=1fio --name=job-randw --rw=randwrite --size=$SIZE --ioengine=libaio --iodepth=32 --bs=4k --direct=1
```
...
![Storage Benchmark 2/2](https://chollinger.com/_astro/image-20240825134453828.07ugydpf.png)

Storage Benchmark 2/2.

(Credit: [by me](https://chollinger.com/))

### Blob storage, blob fish, and file systems: It’s all “meh”[↑](#top "Back to top")
...
![Nextcloud on iOS](https://chollinger.com/_astro/image-20240825114612035.BoB04VzP.png)
