...
captured_at: "2026-09-25T22:33:30+01:00"
title: "Unit Testing Clean Architecture Use Cases"
domain: "milanjovanovic-tech"
---

Unit testing a Clean Architecture use case means mocking every dependency the handler takes, then asserting on the `Result` it returns. Cover the failure paths, the happy path, and exception propagation, using Arrange-Act-Assert and descriptive test names. A mock cannot prove that the real query logic works, which is what integration tests are for.
...
I've seen many projects fail because of poor testing practices. The codebase grows, changes become risky, and developers lose confidence in their deployments. This is especially true for [**Clean Architecture**](https://www.milanjovanovic.tech/blog/clean-architecture-dotnet) projects, where we need to ensure our use cases work correctly.
...
## [Understanding Different Testing Approaches](#understanding-different-testing-approaches)
...
1.  [**Unit tests**](https://www.milanjovanovic.tech/blog/unit-testing-best-practices-dotnet) focus on testing individual components in isolation. They're fast, reliable, and help you catch issues early. I write these tests first, and they make up the majority of my test suite.
    
2.  [**Integration tests**](https://www.milanjovanovic.tech/blog/testcontainers-integration-testing-using-docker-in-dotnet) verify that different components work together correctly. They're slower but essential for testing database operations or external services.
    
3.  [**End-to-end tests**](https://www.milanjovanovic.tech/blog/testing-modular-monoliths-system-integration-testing) check the entire system flow. They're the slowest but provide confidence that everything works together.
...
## [Breaking Down Our Use Case](#breaking-down-our-use-case)
...
## [Setting Up Our Test Environment](#setting-up-our-test-environment)
...
I'm using [NSubstitute](https://nsubstitute.github.io/) to create mocks of our dependencies. Each test starts with fresh mocks, preventing test interference. The static fields provide consistent values across all tests.
...
## [Testing the Not Found Scenario](#testing-the-not-found-scenario)
...
I use [FluentAssertions](https://fluentassertions.com/) because it provides clear, readable assertions and better error messages than the standard Assert class.

## [Handling Booking Conflicts](#handling-booking-conflicts)
...
## [Testing Successful Bookings](#testing-successful-bookings)
...
## [Understanding Test Coverage Limitations](#understanding-test-coverage-limitations)
...
How do you unit test a Clean Architecture use case?

Mock every external dependency, such as repositories and a date-time provider, with a library like NSubstitute, then test the handler in isolation. Cover failure paths, the happy path, and exception propagation, using the Arrange-Act-Assert pattern and descriptive test names.

Why shouldn't you use DateTime.UtcNow directly in production code?

Code that reads DateTime.UtcNow directly is much harder to test because you cannot control the time. Inject a date-time provider abstraction instead; in tests you mock it to return a fixed value, which makes time-dependent behavior deterministic and easy to verify.

What is the Arrange-Act-Assert pattern?

Arrange-Act-Assert structures every test in three steps: arrange the inputs and mock behavior, act by invoking the code under test, and assert the outcome. It keeps tests consistent and readable, and it maps naturally onto testing command handlers.

What can't unit tests verify in Clean Architecture?

Unit tests verify the business logic flow, but a mocked repository only returns whatever you tell it to. They cannot prove that logic like booking overlap detection works against real data; that requires integration tests that insert real records into a test database.

What is the difference between unit, integration, and end-to-end tests?

Unit tests check individual components in isolation; they are fast and reliable and should make up the majority of your suite. Integration tests verify components working together, such as database operations. End-to-end tests cover the whole system flow and are the slowest.

Does high code coverage mean good tests?

No. Code coverage does not directly translate to good tests, though it is a useful starting point. It is up to you to write quality tests that cover the most critical parts of your system rather than chasing a percentage.
