...
sources:
  ...
  - title: "Raw clipping (archived copy)"
    url: "https://github.com/thluiz/scholion/blob/main/clippings/2026-09/dev-to--top-5-ai-models-you-can-run-locally.md"
    kind: repo
---
...
- The text says that many AI APIs send messages in plain text and that some companies collect conversation data or conduct human review of chats.
- For local use, the author chooses GPT4All because it is easy to use and has cross-platform support, instead of a CLI like Ollama.
- The article warns that the models can take up from 2 to 6 GB and that it is necessary to check disk space before downloading.
- Nous Hermes 2 Mistral DPO is presented as a general, fast, fine-tuned model with about 7 billion parameters.
- Qwen2-1.5B-Instruct is recommended for weak devices, but the text warns that it can hallucinate easily.
- Llama 3 8B Instruct is described as an open source model, with good quality and a friendly personality.
- Mini Orca (Small) is suggested as a small and fast model, more useful for explanations than for conversation, and also prone to hallucinations, especially in mathematics.
