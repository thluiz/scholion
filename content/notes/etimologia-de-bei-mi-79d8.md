...
Composto fonossemântico (no caractere atual): 禾 (hé, "cereal" — radical convencional) + 必 (bì, fonético).
A forma original (祕, U+7955) tem o radical 礻/示 (shì, "sinal divino/altar") — semanticamente correto. A migração de 礻 para 禾 é provavelmente erro de transmissão estabilizado por uso popular.
Decomposição (forma 秘 atual): 禾 + 必. Decomposição (forma 祕 original): 示 (semântico) + 必 (fonético).
