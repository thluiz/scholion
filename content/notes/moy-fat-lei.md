---
title: "梅法利 Moy Fat Lei"
date: 2026-04-12T10:00:00+01:00
summary: "Etimologia do nome kung fu de Thiago Pereira: Fat (法) lei, método, Lei (利) benefício, afiado."
tags: ["ving-tsun", "etimologia", "kung-fu"]
has_commentary: true
sources:
  - title: "MDBG Chinese Dictionary"
    url: "https://www.mdbg.net/chinese/dictionary"
    kind: wiki
  - title: "Priberam Dicionário"
    url: "https://dicionario.priberam.org"
    kind: wiki
  - title: "Chinese Etymology (hanziyuan.net)"
    author: "Richard Sears"
    url: "https://hanziyuan.net"
    kind: wiki
---

**梅法利**
*Moy Fat Lei*

Nome kung fu de Thiago Pereira.

**Fat** (法) – Lei, método, caminho.

**Lei** (利) – Benefício, lucro, afiado.

## Etimologia no Chinês

### 法

氵(shuǐ / seoi2) – água
去 (qù / heoi3) – ir, partir

A água que segue seu curso — o método natural. O Shuowen Jiezi define 法 como 刑也, "norma, penalidade", e registra a forma antiga com o componente 廌 (zhì), um animal mítico capaz de distinguir o justo do injusto. A água é o modelo: segue sempre o caminho mais direto, sem desvio. Lei e método nascem da mesma imagem — o fluxo que não se contradiz.

### 利

禾 (hé / wo4) – grão, cereal
刂 (dāo / dou1) – faca

Colher o grão com a faca — o benefício. O Shuowen Jiezi define 利 como 銛也, "afiado". A lâmina que ceifa a colheita é o gesto do proveito: o que é afiado é útil, o que é útil é proveitoso.
